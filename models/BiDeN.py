...
class MultipleChoiceModel(pretrained_model_class):
    ...
    def forward(
        ...
        if training:
            ...
            if rank is None and not args.with_rank:
                choice_loss = choice_loss_fct(logits, answer_ids)
            else:
                # choice_loss = (1-args.loss_portion)*choice_loss_fct(logits, answer_ids)+\
                #               args.loss_portion*(neuralNDCG(F.softmax(logits), rank, temperature=args.neural_ndcg_temp)+torch.tensor(1.).cuda())
                if choice_loss_fct(logits, answer_ids) >= (neuralNDCG(F.softmax(logits), rank, temperature=args.neural_ndcg_temp)+torch.tensor(1.).cuda()):
                    loss_type = 'CE'
                else:
                    loss_type = 'nNDCG'
                choice_loss = torch.max(choice_loss_fct(logits, answer_ids),
                              neuralNDCG(F.softmax(logits), rank, temperature=args.neural_ndcg_temp)+torch.tensor(1.).cuda())
        else:
        ...
        outputs = ((choice_loss, 'CE') if rank is None and not args.with_rank else (choice_loss, loss_type))if training else (pred_dict,)
        return outputs
