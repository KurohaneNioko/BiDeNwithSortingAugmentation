...
class Example(object):
    def __init__(self, utterances, options, qid, answer=None, rank=None):
...
class InputFeature(object):
    def __init__(self, qid, ids_dict, sep_poses, piece_idxes_list, answer_ids=None, rank=None):
        self.qid = qid
        self.ids_dict = ids_dict
        self.sep_poses = sep_poses
        self.piece_idxes_list = piece_idxes_list
        self.answer_ids = answer_ids
        self.rank=rank # list: [3,2,1,0] == A>B>C>D
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if args.with_rank and self.features[index].rank is not None:  # not None for eval/test
            data_info['rank'] = torch.tensor(self.features[index].rank, dtype=torch.float32)
        front_masks = []
        tail_masks = []
        self_masks = []
        fusion_masks = []
...
def read_examples(input_file, training=True):
    def rank2list(ranks, d_id):
        permu = ranks[d_id].split('>')
        r = [0]*len(permu)
        for i in range(len(permu)):
            r[answer_to_id[permu[i]]] = len(permu)-1-i
        return r
    ...
    if args.with_rank:
        with open(os.path.join(args.data_path, args.dataset+'_aug', args.dataset+'_aug.json'), "r", encoding='utf-8') as rank_reader:
            ranks = json.load(rank_reader)
    ...
    for dialogue in tqdm(input_data, ncols=100):
        ...
        rank = rank2list(ranks, qid) if args.with_rank and 'train' in input_file else None
        exp = Example(utterances, options, qid, answer, rank=rank)
        examples.append(exp)
...
def convert_examples_to_features(examples, tokenizer, max_length, training=True):
    ...
    for exp in tqdm(examples, ncols=100):
        ...
        if not training:
            ...
            continue
        # training
        f_tmp = InputFeature(exp.qid, ids_dict, sep_poses, piece_idxes_list, exp.answer, exp.rank)
        features.append(f_tmp)
