...
def train(model, train_loader, eval_dataloader, test_dataloader, tokenizer):
    ...
    # logging_step = len(train_loader) // 4   # for 7088/886
    logging_step = len(train_loader) // 8     # for 7088/443
    steps = 0
    ...
    for epoch in range(args.epochs):
        ...
        for _, batch in pbar:
            ...
            if steps != 0 and steps % logging_step == 0:
                ...
                # evaluate(model, test_dataloader, tokenizer, best_result, is_test=True)
                my_test(model, test_dataloader)  # my modify: for test from original dev
            steps += 1

    evaluate(model, eval_dataloader, tokenizer, best_result, is_test=False)
    my_test(model, test_dataloader)
...
def evaluate(model, eval_loader, tokenizer, cur_best_result=None, is_test=False, file_name=None):
    ...
    if file_name is None and not is_test:
        ...
        for qid in pred_dict.keys():
            prob_dict = pred_dict[qid]
            pred_choices = list(dict(sorted(prob_dict.items(), key=lambda x: x[1], reverse=True)).keys())
            line = qid + '\t' + pred_choices[0] + '\t' + pred_choices[1] + '\t' + pred_choices[2] + '\t' + pred_choices[
                3] + '\n'
            lines.append(line)
...
def my_test(model, eval_loader):
    ...
    with open(test_path, "r", encoding='utf-8') as reader:
        golden_dict = {d['id']: d['answers'] for d in json.load(reader)}
    r1, r2, mrr = _cal_metrics(golden_dict, pred_dict)
    result_dict = {'r1': r1, 'r2': r2, 'mrr': mrr}
    with open('total_result.txt', 'a+', encoding='utf-8') as result_writer:
        result_writer.write(str(args) + '\n')
        result_writer.write("Eval result: " + str(result_dict) + '\n\n')
        print("Test result:", result_dict)

    model.train()
    return result_dict


def rank_train(model, train_loader, eval_dataloader, test_dataloader, tokenizer):
    print("Traning arguments:")
    print(args)
    ...
    for epoch in range(args.epochs):
        ...
        for _, batch in pbar:
            ...
            if len(outputs) != 4:
                pbar.set_description("Loss:%.3f,CL:%.3f,LossType:%s" % (loss.item(), loss.item(), outputs[1]))
            elif len(outputs) == 4:
                token_level_loss, utter_level_loss, ori_out_loss = outputs[1], outputs[2], outputs[3]
                pbar.set_description("Loss:%.3f,TCL:%.3f,UCL:%.3f,OCL:%.3f" % (
                    loss.item(), token_level_loss, utter_level_loss, ori_out_loss))
...
def original_run():
    ...
    # args.with_rank = True
    # if args.with_rank:
    #     args.neural_ndcg_temp = 0.05
    #     args.loss_portion = 0.5  # portion of neural_ndcg
    if is_eval:
    ...
    if not is_eval:
        train_dataset = get_dataset(train_path, args.cache_path, tokenizer, args.max_length, training=True)
    ...
    if not is_eval:
        if not args.with_rank:
            ...
            rank_train(model, train_dataloader, eval_dataloader, test_dataloader, tokenizer)
    else:
...
if __name__ == "__main__":
    ...
    args.epochs = 6
    print('without rank starting')
    for dataset_names in [ 'mutual_plus']:
        args.dataset = dataset_names
        set_cfg_by_args()
        original_run()    # original loss
        # exit(0)
    print('with rank starting')
    args.with_rank = True
    temps = [0.001, 0.01, 0.05, 0.1, 0.5, 0.75, 1]
    # temps = [0.001, 0.01, 0.05]
    # loss_portions = [0.1, 0.3, 0.5, 0.7, 1]
    shutil.rmtree('./caches', ignore_errors=True)
    # for temperature, loss_portion in itertools.product(temps, loss_portions):
    for temperature in temps:
        # change to max of the 2 loss
        # for loss_portion in loss_portions:
        for dataset_names in ['mutual']:
            args.neural_ndcg_temp = temperature
            # args.loss_portion = loss_portion
            # print(f'Temp={temperature}, neuralNDCG={loss_portion}')
            args.dataset = dataset_names    # set dataset name
            set_cfg_by_args()               # go through config.py
            ...
            model = model.to(device)
            rank_train(model, train_dataloader, eval_dataloader, test_dataloader, tokenizer)
            del train_dataset, eval_dataset, test_dataset
